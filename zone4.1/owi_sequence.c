...
typedef enum{
	...
	ARM_DWN  = 0x000004 | 0x000020 | 0x000080 | 0x000100, // wrist up   + elbow down + shoulder down + base clockwise
} cmd;
...
static struct sequence_step main_sequence[] = {
	...
	{ .command = STOP, 				.duration_ms = T_STOP },
	{ .command = GRIP_CLOSE, 		.duration_ms = T_GRIP },
	{ .command = BASE_COUNTERCLK, .duration_ms = T_BASE+T_ARM-250},
...
static struct sequence_step unfold_sequence[] = {
	...
	{ .command = SHOULDER_DWN, .duration_ms = T_FOLD_SHOULDER + 50},
